...
namespace Domain
{
    public class ReferenceData : IReferenceData
    {
        ...
        async Task AddBrand(string brandName, int productCount)
        {
            var brand = await Create(new Brand { Name = brandName });

            for (int counter = 0; counter < productCount; counter++)
                await Create(new Product { Title = $"{brandName} - {counter + 1}", Brand = brand });
        }
